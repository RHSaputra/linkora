...
import { getCategoryColor, cn, formatRelativeTime } from "@/lib/utils";
...
export function LinkCard3D({ link, index, onUpdate, onDelete, onEdit, viewMode = "detail" }: LinkCard3DProps) {
  ...
  return (
    <motion.div
      layout
      layoutId={`card-3d-container-${link.id}`}
      transition={{
    ...
    >
      <Card3D className="h-full flex flex-col">
        <div 
          ...
          className={cn(
            "group h-full flex flex-col justify-between glass-panel rounded-2xl overflow-hidden cursor-pointer relative border border-border/80 bg-card/90 transition-all duration-300",
            isCompact ? "p-2.5 sm:p-3 rounded-xl hover:shadow-md" : "hover:shadow-lg"
          )}
          style={{ transform: isCompact ? undefined : "translateZ(30px)" }}
        >
          {/* Glass shimmer sweep on mode change */}
          ...
          {isCompact ? (
            <div className="flex items-center justify-between gap-3 w-full">
              ...
              <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                <motion.div
                  layoutId={`favicon-3d-box-${link.id}`}
