...
export function SmartSearchDemo() {
  ...
  return (
    <section id="demo" className="py-10 md:py-18 relative overflow-hidden bg-background">
      {/* Glow aura */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] sm:w-[650px] h-[340px] sm:h-[650px] bg-primary/15 rounded-full blur-[80px] sm:blur-[140px] pointer-events-none" />
      ...
      <div className="container px-3.5 sm:px-6 relative z-10">
        ...
        >
          ...
          <div className="flex items-center justify-center gap-1 sm:gap-2 mb-3 sm:mb-4 flex-wrap">
            {searchExamples.map((ex, idx) => (
              <button
                ...
                className={`px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-medium transition-all cursor-pointer touch-manipulation select-none ${
                  currentIndex === idx
          ...
          <div className="glass-panel rounded-xl sm:rounded-3xl p-2.5 sm:p-5 border border-primary/30 shadow-2xl shadow-primary/15 relative overflow-hidden">
            {/* Search Input Simulation */}
            <div className="flex items-center gap-2 sm:gap-3 bg-foreground/[0.04] rounded-lg sm:rounded-2xl p-2.5 sm:p-4 border border-border/60">
              <Search className="w-4 h-4 sm:w-6 sm:h-6 text-primary shrink-0 animate-pulse" />
              <div className="flex-1 overflow-hidden relative h-6 sm:h-8 flex items-center">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="absolute text-xs sm:text-lg md:text-xl font-medium text-foreground flex items-center gap-1.5 sm:gap-2"
                  >
                    <span>{current.query}</span>
                    <span className="w-0.5 h-3.5 sm:h-5 bg-primary animate-pulse" />
                  </motion.div>
                </AnimatePresence>
              </div>
            </div>
            ...
            <div className="mt-2.5 sm:mt-4 px-0.5 sm:px-1">
              <div className="flex items-center justify-between text-[10px] sm:text-xs text-muted-foreground mb-2 sm:mb-3 font-medium">
              ...
              <div className="space-y-1.5 sm:space-y-2">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="flex items-center justify-between p-2.5 sm:p-4 rounded-lg sm:rounded-2xl bg-foreground/[0.03] hover:bg-foreground/[0.06] transition-all cursor-pointer border border-border/60 group"
                  >
                    <div className="flex items-center gap-2 sm:gap-3.5 min-w-0 flex-1">
                      <div className={`p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl border ${current.color} shrink-0`}>
                        <Icon className="w-3.5 h-3.5 sm:w-5 sm:h-5" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 sm:gap-2">
                          <span className="font-bold text-foreground text-[11px] sm:text-sm truncate">{current.query}</span>
                          <span className="text-[8.5px] sm:text-[10px] px-1 sm:px-2 py-0.5 rounded-md bg-primary/10 text-primary font-semibold shrink-0">
                            {current.tag}
                          </span>
                        </div>
                        <div className="text-[10px] sm:text-xs text-muted-foreground mt-0.5 truncate">{current.result}</div>
                      </div>
                    </div>
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-foreground/5 flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground transition-colors shrink-0 ml-1.5 sm:ml-2">
