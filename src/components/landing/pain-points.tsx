...
const problems = [
  { 
    text: "Link magang tersimpan di WhatsApp & chat personal.", 
    icon: MessageCircle, 
  ...
  { 
    text: "Link beasiswa tersimpan di Telegram / bookmark browser.", 
    icon: Send, 
  ...
  { 
    text: "Link lomba & kompetisi tertumpuk di tab tak terbaca.", 
    icon: Bookmark, 
  ...
  { 
    text: "Catatan tugas & materi riset terpencar di banyak aplikasi.", 
    icon: FileText, 
  ...
  { 
    text: "Saat dibutuhkan segera, semua hilang dan sulit dicari.", 
    icon: AlertCircle, 
...
const FULL_SUBTITLE = "Informasi berharga tersebar di berbagai sudut digital, membuat Anda membuang jam kerja berharga hanya untuk mencari kembali tautan lama."
...
function TypewriterSubtitle() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      currentIndex++
      setDisplayedText(FULL_SUBTITLE.slice(0, currentIndex))
      if (currentIndex >= FULL_SUBTITLE.length) {
  ...
  return (
    ...
    >
      {displayedText || (isInView ? "" : FULL_SUBTITLE)}
      {!isDone && isInView && (
...
export function PainPoints() {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // DETEKSI STEP DENYUT KONSISTEN & DISKRIT (Mencegah rasa kagok/inkonsistensi)
  // ─────────────────────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 4-STAGE CINEMATIC SEQUENCE (Total 500vh):
  //
  // TAHAP 1 (0.00 – 0.55): 5 Card di Tengah dengan Active Focus Step yang Konsisten
  // TAHAP 2 (0.55 – 0.66): 5 Card meluncur mulus ke Samping Kiri
  // TAHAP 3 (0.66 – 0.78): Card Besar muncul di Samping Kanan
  // TAHAP 4 (0.78 – 0.90): Saling menumpuk ke Tengah (Hanya 5 Ikon Badge yang Terlihat di Kiri)
  // FINAL STATE (0.90 – 1.00): Statis Penuh
  // ─────────────────────────────────────────────────────────────────────────
  ...
  // Teks dan tombol X memudar menjadi 0 saat proses menumpuk
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6 lg:gap-10 items-stretch max-w-5xl mx-auto relative">
            ...
            >
              {problems.map((problem, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Badge Ikon: Selalu 100% Terlihat & Menyembul di Sisi Kiri */}
                    ...
                    {/* Teks: Memudar Mulus saat Menumpuk agar Tidak Ada Potongan Teks */}
                    <motion.p 
                      style={{ 
                    ...
                    {/* Ikon X: Memudar Mulus saat Menumpuk */}
                    <motion.div
                      style={{ 
                        opacity: cardTextOpacity,
                        willChange: "opacity" 
            ...
            >
              ...
              <div className="relative z-10 space-y-4 sm:space-y-6 w-full max-w-md mx-auto">
                {/* Checkmark dengan dual-stage pulse "approved" */}
                <motion.div
                  style={{
                    scale: checkScale,
                    boxShadow: checkGlow,
                ...
                <div className="space-y-2 sm:space-y-3">
                  <h3 className="text-lg sm:text-2xl md:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                    <LinkoraText /> Menyatukan Semuanya.
                  </h3>
